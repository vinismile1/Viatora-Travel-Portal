...
export function RestaurantCard({ restaurant }: RestaurantCardProps) {
  ...
  return (
    <div id={`restaurant_card_${restaurant.id}`} className="flex flex-col bg-theme-card rounded-[2rem] border border-theme-border overflow-hidden shadow-2xl hover:border-primary/30 transition duration-300 relative text-theme-text">
      {/* Save Button */}
      <button
      ...
      <div className="h-48 overflow-hidden relative bg-theme-panel">
        ...
        />
        <div className="absolute top-3 left-3 bg-theme-card/85 backdrop-blur-md px-2.5 py-0.5 rounded-full text-[10px] font-bold text-theme-text border border-theme-border">
      ...
      <div className="p-5 flex-1 flex flex-col justify-between">
        <div>
          <div className="flex justify-between items-start mb-1">
            <h3 className="font-display font-semibold text-theme-text text-base md:text-lg">{restaurant.name}</h3>
          </div>
          <p className="text-emerald-500 text-xs font-medium mb-2.5">{restaurant.cuisine}</p>
          <p className="text-theme-muted text-xs md:text-sm mb-4 line-clamp-2">{restaurant.description}</p>
        ...
        <div>
          <div className="flex items-center justify-between pt-3 border-t border-theme-border">
            <span className="text-xs text-theme-muted font-sans">Highly Rated Dining</span>
            <button
              id={`book_restaurant_modal_btn_${restaurant.id}`}
              onClick={() => setShowBookingModal(true)}
              className="px-4 py-2 bg-emerald-600 text-white text-xs font-semibold rounded-xl hover:bg-emerald-700 shadow-md transition cursor-pointer"
            >
              Book Table
            </button>
          </div>
      ...
      {showBookingModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-theme-card border border-theme-border rounded-2xl shadow-2xl max-w-sm w-full p-6 animate-in fade-in zoom-in duration-200">
            <h3 className="font-display font-semibold text-lg text-theme-text mb-1">Reserve Table: {restaurant.name}</h3>
            <p className="text-xs text-theme-muted mb-4">Secures a table reservation for 2. Small reservation deposit applies.</p>
              ...
              <form onSubmit={handleBook} className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="relative flex items-center">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-3 py-2 bg-theme-panel border border-theme-border rounded-xl outline-none focus:border-emerald-500 text-sm text-theme-text"
                      />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-theme-muted mb-1">Time</label>
                    <div className="relative flex items-center">
                      <Clock size={16} className="absolute left-3 text-theme-muted" />
                      <input
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    type="submit"
                    className="w-1/2 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-semibold shadow-md transition cursor-pointer"
                  >
                    Confirm Table
                  </button>
                </div>
